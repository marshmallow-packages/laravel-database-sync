...
class DumpDataAction
{
    public static function handle(
        ...
        $dumpCommand = "mysqldump -u {$config->remote_database_username} -p{$config->remote_database_password} {$dump_flags} --where='id IN (SELECT id FROM {$table} WHERE created_at >= \\\"{$config->date} 00:00:00\\\" OR updated_at >= \\\"{$config->date} 00:00:00\\\")' {$config->remote_database} {$table}";
